...
class IP(object):
    ...
    def __init__(self, ip_address: str, current_port: int = 0,
        ...
        self._max_payload = MAXPAYLOAD  # 一个主机下最大运行客户端的数量,一个ip下开的最多的容器（节点）数量,一个容器对应一个节点
        self.current_port = current_port
        self.address = ip_address
        self.rpc_ports = range(8515, 8515 + self.max_payload * 10, 10)  # rpc端口 8515 8525,每一个ip都从8515开始
    ...
    def __init__(self, ip_file: str, current_ip: int = 0,
        ...
        with open(ip_file, 'r') as f:
            ...
            for line in f.readlines():
                if line.strip():
                    self.ips.append(IP(line.strip()))  # 创建IP实例，存放在_ips中
                else:
                    break
        self._init_service()
